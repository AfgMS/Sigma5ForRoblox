...
local function GetMatchState()
    return Bedwars["ClientStore"]:getState().Game.matchState
end

function getQueueType()
    local MatchState = Bedwars["ClientStore"]:getState()
...
local function SetHotbar(item)
    if localPlayer.Character:FindFirstChild("HandInvItem").Value ~= item then
        ...
        Bedwars["SetItem"]:InvokeServer({["hand"] = item})
    end
end
...
local function GetRanged()
    ...
    for i, v in pairs(localPlayer.Character.InventoryFolder.Value:GetChildren()) do
        if v.Name:match("bow") or v.Name:match("fire") or v.Name:match("head") then
            for _, data in pairs(RangedRank) do
                if data.Name == v.Name then
                    if bestrank <= data.Rank then
                        bestrank = data.Rank
                        bestProject = v
        ...
        if enabled then
            ...
            if AutoGG then
                repeat
                    task.wait(1)
                until GetMatchState() == 2 or not enabled
                Bedwars["ChatService"]:FireServer("GG", "All")
            end
        else
